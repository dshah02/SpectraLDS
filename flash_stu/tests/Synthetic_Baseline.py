...
parser.add_argument('--lr', type=float, default=1e-4, help='Delta parameter for LDS')
parser.add_argument('--seq_len', type=int, default=8192, help='Delta parameter for LDS')
parser.add_argument('--hidden_state_expansion', type=float, default=1, help='Learning with more room')
...
d_out = args.d_in
dtype = torch.float32
use_zero_hidden = False
seq_len = args.seq_len


model = LDS(d_h, d_in, d_out).to(device)
lr = args.lr
steps = args.steps
delta = args.delta
bsz = 32

output_dir = f"synth_results/baseline/{delta}_{d_in}_{d_h}/"
...
with torch.no_grad():
    ...
    # Plot results
    fig, (ax2, ax3) = plt.subplots(2, 1, figsize=(12, 12), sharex=True)
